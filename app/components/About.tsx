...
const About: React.FC = () => {
  ...
  const executiveMembers = leaders.filter(leader => leader.position === 'Executive Committee Member');
  const actionCouncilMembers = leaders.filter(leader => leader.position === 'Action Council Member');
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="p-4 sm:p-8">
            <p className="text-lg text-gray-700 leading-relaxed mb-8">
              ...
              {!showFullText && words.length > 60 && (
                <span onClick={() => setShowFullText(true)} className="text-base font-semibold hover:text-gray-700 text-teal-600 cursor-pointer">
                  read more
                </span>
              )}
              {showFullText && (
                <span onClick={() => setShowFullText(false)} className="hover:text-gray-700 text-teal-600 cursor-pointer text-base font-semibold">
                  read less
                </span>
              )}
            </p>

            <div onClick={() => setShowBoardMembers(!showBoardMembers)} className='shadow cursor-pointer flex justify-between text-black w-full font-semibold p-2 rounded-lg hover:bg-teal-700 hover:text-white transition duration-300'>
          ...
          {showBoardMembers && (
            <div className="mt-12">
              <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6 text-center">Board Members</h3>
              ...
              {executiveMembers.length > 0 && (
                <>
                  <h4 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-4">Executive Committee Members</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {executiveMembers.map((leader, index) => (
                      <div
                        key={index}
                        className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden"
                      >
              ...
              {actionCouncilMembers.length > 0 && (
                <>
                  <h4 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-4">Action Council Members</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {actionCouncilMembers.map((leader, index) => (
                      <div
                        key={index}
                        className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden"
                      >
